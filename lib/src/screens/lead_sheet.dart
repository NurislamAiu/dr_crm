...
class _LeadSheetState extends ConsumerState<LeadSheet> {
  ...
  void initState() {
    ...
    if (ex != null) {
      ...
      _currency = ex.currency ?? defaultCurrency(ex.phone);
      _nextNumber = _dayIndexOf(ex);
    } else {
      if (widget.prefillName != null) _name.text = widget.prefillName!;
      if (widget.prefillPhone != null) _phone.text = widget.prefillPhone!;
      _currency = defaultCurrency(widget.prefillPhone);
      _nextNumber = _todayCount() + 1;
    }
  ...
  int? _dayIndexOf(Lead ex) {
    ...
    final i = sameDay.indexWhere((l) => l.id == ex.id);
    return i >= 0 ? i + 1 : null;
  }
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      _card(dark, 'Предоплата', Icons.payments_rounded, [
                        _field(dark, _prepay, 'Сумма предоплаты (${currencySymbol(_currency)})',
  ...
  Widget _dateTile(bool dark, String label, DateTime? value, ValueChanged<DateTime?> onPick) {
    return _pickTile(dark,
      ...
      final res = await showSoftDatePicker(context, selected: value, accent: _teal);
    ...
    return _pickTile(dark,
      ...
      final t = await showTimePicker(
        ...
        builder: (ctx, child) => Theme(
          data: Theme.of(ctx).copyWith(colorScheme: Theme.of(ctx).colorScheme.copyWith(primary: _teal)),
